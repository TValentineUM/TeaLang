...
The implementation closely follows the one laid out in [@eac-parser]. The implementation works by creating parse functions for the various construction rules inside the BNF. When parsing the FIRST set is used to determine the correct production, with an example first set for `parse_statement` found below.
...
The Abstracy Syntax tree for Tealang is built top-down during parsing. The tree is built to leverage OOP poly-morphism, in this was a statement can contain an expression without needing to store the specific expression type. 
